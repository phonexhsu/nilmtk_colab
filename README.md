...
訪問http://{repasberrypi-ip}:1880就可以進到node-red的flow editor。然後請安裝 [node-red-contrib-mongodb2 (node) - Node-RED](https://flows.nodered.org/node/node-red-contrib-mongodb2) 與 [node-red-contrib-modbus (node) - Node-RED](https://flows.nodered.org/node/node-red-contrib-modbus) 還有 https://flows.nodered.org/node/node-red-dashboard 3個packages。如何安裝package請參考 [Node-Red Palette, Importing Nodes - YouTube](https://www.youtube.com/watch?v=Wlwe5Xry5cA)。
